...
public static class MethodBaseExtensions
{
    ...
    public static bool HasParameters(this MethodBase methodInfo, params Func<ParameterInfo, bool>[] predicates)
    {
        if (methodInfo is null) throw new ArgumentNullException(nameof(methodInfo));
        if (predicates is null) throw new ArgumentNullException(nameof(predicates));

        var parameters = methodInfo.GetParameters();
        if (parameters.Length != predicates.Length) return false;

        for (var i = 0; i < parameters.Length; i++)
        {
            if (!predicates[i](parameters[i])) return false;
        }
        return true;
    }
